...
**Level 0 is self-hosted.** The latest release is `2.1.1`, tagged `l0-v2.1.1` in the monorepo's level-prefixed release
namespace. See [l0/docs/releases/2.1.1.md](docs/releases/2.1.1.md) for its complete changes and compatibility guidance.
...
L0 `2.1.1` is the current stable release.

The `2.1.1` release fixes ARC cleanup for comparison temporaries in both compiler stages and improves AddressSanitizer
test validation. It preserves the language, CLI, public C, and standard-library surfaces of 2.1.0. That release added
the declaration-only `dea_rt.h` header, safe byte-vector self-appends, semantic optional hashes, and
AddressSanitizer-visible quarantine lifetimes.
...
Post-L0 language evolution happens primarily in Dea/L1; see [l1/README.md](../l1/README.md). L0 itself is not frozen: it
keeps evolving alongside L1, and new features or changes may, sparingly, be introduced in L0 when they make sense as a
trampoline or foundation for L1. The string value operators (`==`, `<`, `+`, and friends), backported from L1, are an
example of this. Such changes follow the level's semantic versioning and the usual Stage 1/Stage 2 parity discipline.
...
- `case` dispatches on constant values (including strings): `"foobar" => stmt`, or `_ => stmt` for the default. No
...
For the current `2.1.1` support matrix, Windows validation is through MSYS2 `UCRT64` with MinGW-w64 GCC. MSYS2 `MINGW64`
...
[l0/README-WINDOWS.md](README-WINDOWS.md).
...
and [l0/README-WINDOWS.md](README-WINDOWS.md).
...
This program is available in [l0/examples/demo.l0](examples/demo.l0). You can run it from within `l0/` as follows:
...
The authoritative grammar is in [l0/docs/reference/grammar.md](docs/reference/grammar.md).
...
Stage 1 (`compiler/stage1_py`) follows a conventional pipeline from lexer to semantic analysis to C99 code generation.
Stage 2 (`compiler/stage2_l0`) mirrors the same pipeline in L0 itself, with separate lexer, parser, AST, type-checking,
backend, and driver modules.

References:

- [l0/docs/reference/architecture.md](docs/reference/architecture.md): architecture and data flow
- [l0/docs/reference/design-decisions.md](docs/reference/design-decisions.md): design decisions
- [l0/docs/reference/ownership.md](docs/reference/ownership.md): ownership and memory management
- [l0/docs/specs/compiler/stage1-contract.md](docs/specs/compiler/stage1-contract.md): Stage 1 contract
- [l0/docs/specs/compiler/cli-contract.md](docs/specs/compiler/cli-contract.md): shared CLI contract
- [CONTRIBUTING.md](../CONTRIBUTING.md): developer workflow, setup, and validation commands

## Project structure

| Directory                    | Description                                                   |
| ---------------------------- | ------------------------------------------------------------- |
| `compiler/stage1_py/`        | Stage 1 compiler (Python); the bootstrap implementation       |
| `compiler/stage2_l0/`        | Stage 2 compiler (L0); the self-hosted implementation         |
...
| `docs/`                      | Reference documentation, specs, decisions, and project status |
| `scripts/`                   | Utility scripts for development, testing, and maintenance     |
| `tests/`                     | End-to-end tests spanning both stages or external tooling     |
...
Trace output contract: [l0/docs/specs/runtime/trace.md](docs/specs/runtime/trace.md).

## Project status

[l0/docs/project-status.md](docs/project-status.md).
