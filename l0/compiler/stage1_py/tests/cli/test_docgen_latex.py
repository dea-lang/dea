...
def test_normalize_latex_site_splits_front_matter_release_line(tmp_path: Path) -> None:
    xml_dir = tmp_path / "xml"
    xml_dir.mkdir()
    latex_dir = tmp_path / "latex"
    latex_dir.mkdir()

    (latex_dir / "refman.tex").write_text(
        """\\begin{titlepage}
  \\vspace*{7cm}
  \\begin{center}%
  {\\Large Dea/L0 API}\\\\
  [1ex]\\large Generated 2026-03-26 l0docgenlinetwo v0.9.1 (45712f5) \\\\
  \\vspace*{1cm}
...
""",
    ...
    refman = (latex_dir / "refman.tex").read_text(encoding="utf-8")
    assert "[1ex]\\large Generated 2026-03-26 \\\\" in refman
    assert "\\large v0.9.1 (45712f5) \\\\" in refman
    assert "l0docgenlinetwo" not in refman
