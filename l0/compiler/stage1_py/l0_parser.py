...
class Parser:
    ...
    def _parse_case_stmt(self) -> CaseStmt:
        ...
        while not self._check(TokenKind.RBRACE) and not self._at_end():
            if self._check(TokenKind.ELSE):
                self._error("[PAR-0123] 'else' without matching 'if'", self._peek())
                self._sync_case_invalid_arm()
                continue

            # Wildcard default arm: `_ => Stmt`.
            if self._check(TokenKind.UNDERSCORE):
                if seen_default:
                    self._error("[PAR-0236] duplicate '_' default arm in 'case' statement", self._peek())
                    self._sync_case_invalid_arm()
                    continue
            ...
            if seen_default:
                self._error(
                    "[PAR-0234] value arm cannot appear after the '_' default arm in 'case' statement",
                    self._peek())
                self._sync_case_invalid_arm()
                continue
            arm_start = self._span_start()
    ...
    def _at_case_arm_start(self) -> bool:
        ...
        return self._peek().kind in (
            ...
            # `else` is rejected by `_parse_case_stmt`, but preserving it as a
            # boundary lets the loop emit the dedicated PAR-0123 diagnostic.
            TokenKind.ELSE,
            TokenKind.INT,
            TokenKind.BYTE,
            TokenKind.STRING,
