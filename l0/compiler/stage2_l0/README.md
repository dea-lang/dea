...
- `L0_CFLAGS="..."` supplies raw host C compiler flags. Raw runtime definitions take precedence over the helper defaults
  described below.
- `L0_COMPILER_RT_CHECK_BASIC=1` defaults the native compiler binary to basic checked mode; an empty value selects full
  checked mode unless `L0_RT_CHECK_BASIC` or `L0_RT_UNCHECKED` explicitly selects a mode.
- `L0_COMPILER_RT_QUARANTINE_MAX_COUNT=256` bounds the native compiler's quarantine record count; an empty value leaves
  the runtime's normal default in effect. `L0_RT_QUARANTINE_MAX_BYTES` and `L0_RT_QUARANTINE_MAX_COUNT` explicitly
  override compiler-build retention limits.

These settings compile the Stage 2 compiler itself. They do not change the runtime mode or quarantine defaults of
programs later generated by that compiler.
...
non-Windows hosts and `-O2 -static` on Windows before applying the compiler-only runtime defaults above.
