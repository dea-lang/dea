...
The generated `build/dea/bin/l0-env.sh` keeps `L0_HOME` repo-relative, activates the shared monorepo `../.venv` if
present, prepends `build/dea/bin` to `PATH`, and leaves `L0_SYSTEM` unset.
