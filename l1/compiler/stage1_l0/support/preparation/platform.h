...
#if defined(_WIN32)
/** Return whether a Windows executable spelling already contains a suffix. */
static int pc_windows_basename_has_extension(const char *path) {
    const char *base = pc_base(path);
    return strchr(base, '.') != NULL;
}

/** Try one explicitly spelled Windows executable with a PATHEXT suffix. */
static char *pc_windows_executable_variant(const char *name) {
    const char *pathext = getenv("PATHEXT");
    const char *cursor;
    if (pc_windows_basename_has_extension(name))
        return NULL;
    if (!pathext || !*pathext)
        pathext = ".COM;.EXE;.BAT;.CMD";
    for (cursor = pathext;;) {
        const char *delimiter = strchr(cursor, ';');
        size_t length = delimiter ? (size_t)(delimiter - cursor) : strlen(cursor);
        if (length) {
            PcBuffer candidate = {0};
            char *path;
            pc_text(&candidate, name);
            pc_bytes(&candidate, cursor, length);
            path = pc_executable(candidate.s);
            free(candidate.s);
            if (path)
                return path;
        }
...
/** Resolve an explicit Windows executable, including a path without PATHEXT. */
static char *pc_windows_reported_path(const char *name) {
    char *path = pc_executable(name);
    if (!path && (strchr(name, '/') || strchr(name, '\\')))
        path = pc_windows_executable_variant(name);
    return path;
}
#endif

/** Resolve a compiler-reported tool path across native and MSYS spellings. */
static char *pc_reported_executable(const char *name, const char *compiler) {
    char *path;
    if (!name || !*name)
        return NULL;
#if defined(_WIN32)
    const char *base;
    path = pc_windows_reported_path(name);
    base = pc_base(name);
    if (!path && compiler && *base && strcmp(base, ".") && strcmp(base, "..")) {
        char *parent = pc_parent(compiler), *candidate = pc_join(parent, base);
        path = pc_windows_reported_path(candidate);
        free(parent);
        free(candidate);
    }
    if (!path && base != name && *base)
        path = pc_windows_reported_path(base);
#else
    (void)compiler;
    path = pc_executable(name);
#endif
    return path;
}
...
static PcProbe pc_probe(const PcJson *words, int timeout_ms) {
    ...
    if (!failed) {
        ...
        started = CreateProcessA(argv[0], command.s, NULL, NULL, TRUE,
