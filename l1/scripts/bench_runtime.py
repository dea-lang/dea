...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Comma-separated DEA_RT_QUARANTINE_MAX_COUNT values",
    )
...
def run_scenario(exe: Path, scenario: str, scale: int, quarantine_count: int | None) -> dict[str, float]:
    env = dict(os.environ)
    if quarantine_count is not None:
        env["DEA_RT_QUARANTINE_MAX_COUNT"] = str(quarantine_count)
...
def bench_cell(exe: Path, scenario: str, scale: int, runs: int, quarantine_count: int | None) -> dict[str, float]:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for cc in compilers:
            ...
            unchecked_exe = work_dir / f"bench_{cc.replace('/', '_')}_unchecked"
            compile_variant(cc, "-DDEA_RT_UNCHECKED", unchecked_exe)
            cells = {
                scenario: bench_cell(unchecked_exe, scenario, args.scale, args.runs, None)
            ...
            for setting in settings:
                cells = {
                    scenario: bench_cell(checked_exe, scenario, args.scale, args.runs, setting)
                    for scenario in SCENARIOS
                }
                print(format_row(str(setting), cells))
    return 0
