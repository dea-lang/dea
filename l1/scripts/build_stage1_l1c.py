...
def stage1_support_args(*, preparation: bool = True) -> list[str]:
    """Return structured compiler arguments for the required support units.

    Args:
        preparation: Include the native-preparation ABI. Compiler builds keep
            the default; implementation tests declare whether they require it.

    Returns:
        Ordered additional C source arguments, without changing compiler flags.
    """

    return [arg for source in STAGE1_SUPPORT_SOURCES
            if preparation or source.name != "preparation_support.c"
            for arg in ("--c-source", str(source))]
