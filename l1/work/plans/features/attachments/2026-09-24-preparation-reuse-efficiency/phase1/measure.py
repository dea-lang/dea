...
"""Run isolated Phase 1 pairs; use the retained image's monotonic timer."""
import json
import pathlib
import re
import shlex
...
report = {'schema': 1, 'repetitions': 5, 'cases': [], 'images': {}, 'resources': {
...
for variant in ('baseline', 'changed'):
    image = f'dea-phase1-{variant}:local'
    ...
    for cc in ('gcc', 'clang'):
        for debug in (True, False):
            for variant in (('baseline', 'changed') if repeat % 2 == 0 else ('changed', 'baseline')):
                ...
                target.chmod(0o777)
                commands = []
                lines = ['set -eu', 'mkdir -p /work/capture', "trap 'cp /work/capture/* /results/' EXIT", 'cat /sys/fs/cgroup/cpu.stat > /work/capture/cpu-before.txt']
                scenarios = [('warmup', []), ('implicit', []),
                             ('explicit', ['--sys-root', '/opt/dea/l1/compiler/shared/l1/stdlib'])]
                if variant == 'changed':
                    scenarios.append(('opt-out', ['--sys-root', '/opt/dea/l1/compiler/shared/l1/stdlib', '--no-managed-stdlib']))
                for scenario, extra in scenarios:
                    for index in (1, 2):
                        ...
                        lines += [shlex.join(timer), f'/work/hello.out > {capture}.program']
                lines += ['cat /sys/fs/cgroup/cpu.stat > /work/capture/cpu-after.txt',
                ...
                argv = ['docker', 'run', '--rm', '--network', 'none', '--read-only', '--memory', '256m',
                        ...
                        f'dea-phase1-{variant}:local', 'sh', '/results/case.sh']
                ...
                for label, scenario, argv in commands:
                    ...
                    if debug:
                        ...
                        source = scenario == 'opt-out' or (variant == 'baseline' and scenario == 'explicit')
                        assert all(e['origin'] == ('source' if source else 'interface') and bool(e['managed']) != source for e in providers)
                        assert len(analyses) == (11 if source else 0), (name, label, analyses)
                ...
                case = {'variant': variant, 'compiler': cc, 'repetition': repeat + 1, 'debug': debug,
                        'records': records, 'cpu_before': (target / 'cpu-before.txt').read_text(),
